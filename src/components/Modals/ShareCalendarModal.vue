  ...
  <v-dialog v-model="show" width="500">
      <v-card>
        <v-toolbar color="indigo" dark>
          <v-toolbar-title>{{ $root.currentMessages.shareCalendar }}</v-toolbar-title>
        </v-toolbar>
        <v-divider></v-divider>
        <v-list>
          <v-list-item>
            <v-list-item-content>
              <v-list-item-subtitle>{{ $root.currentMessages.calendarURL }}</v-list-item-subtitle>
          ...
          <v-list-item>
            <v-list-item-content>
              <v-list-item-subtitle>{{ $root.currentMessages.calendarID }}</v-list-item-subtitle>
      ...
      <v-snackbar v-model="showSnackBar" timeout="2000" bottom>
        {{ $root.currentMessages.copyToClipboard }}
      </v-snackbar>
    </v-dialog>
  </template>
  <script>
